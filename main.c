...
int main(int argc, char *argv[]) {
    ...
    char fileNameEncrypted[100];
    char fileNameDecrypted[100];

    memset(fileNameEncrypted, 0, sizeof(fileNameEncrypted) );
    memset(fileNameDecrypted, 0, sizeof(fileNameDecrypted) );

    /*Concatenate filename to stringsgoo*/
    strcat(fileNameDecrypted, "Decrypted_");
    strcat(fileNameDecrypted, fileName);

    strcat(fileNameEncrypted, "Encrypted_");
    strcat(fileNameEncrypted, fileName);
    ...
    f_enc = fopen(fileNameEncrypted, "wb");
    if (!f_enc) {
    ...
    f_input = fopen(fileNameEncrypted, "rb");
    if (!f_input) {
        /* Unable to open file for reading */
        fprintf(stderr, "ERROR: fopen error: %s\n", strerror(errno));
        return errno;
    ...
    f_dec = fopen(fileNameDecrypted, "wb");
    if (!f_dec) {
        /* Unable to open file for writing */
        fprintf(stderr, "ERROR: fopen error: %s\n", strerror(errno));
        return errno;
